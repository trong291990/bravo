...
<div  class="box-body">
    {{Former::vertical_open()->method("PUT")->action(route('admin.booking.update', $booking->id))->populate($booking)}}
        {{Former::token()}}
        <div class="">
            <div class="col-sm-8">
            ...
            <?php
                $passengers = @json_decode($booking->passenger);
                ...
                <div class="row passenger-item">
                    <div class="col-sm-1">
                        ...
                        <select class="form-control not-picker" name="passengers[title][]">
                            <option  <?php if(@$p->title==='Mr') echo 'selected="selected"' ?> value="Mr">Mr</option>
                            <option  <?php if(@$p->title==='Mrs') echo 'selected="selected"' ?> value="Mrs">Mrs</option>
                            <option  <?php if(@$p->title==='Miss') echo 'selected="selected"' ?> value="Miss">Miss</option>
                            <option  <?php if(@$p->title==='Sir') echo 'selected="selected"' ?> value="Sir">Sir</option>
                        </select>
                    </div>
                    <div class="col-sm-2">
                        <label>Sub name</label>
                        <input value="{{@$p->sub_name}}" type="text" name="passengers[sub_name][]" class="form-control" />
                    </div>
                    <div class="col-sm-3">
                        <label>First and middle name</label>
                        <input value="{{@$p->first_middle_name}}"  type="text" name="passengers[first_middle_name][]" class="form-control" />
                    </div>
                    <div class="col-sm-2">
                        <label>Date of birthday</label>
                        <input value="{{@$p->birthday}}"   type="text" name="passengers[birthday][]" class="form-control datepicker" />
                    </div>
                    <div class="col-sm-2">
                        <label>PP Number</label>
                        <input value="{{@$p->pp_number}}"   type="text" name="passengers[pp_number][]" class="form-control" />
                    </div>
                    <div class="col-sm-2">
                        <label>Nationality</label>
                        <input value="{{@$p->nationality}}"   type="text" name="passengers[nationality][]" class="form-control" />
                    </div>
                </div>
                @endforeach
            </div>
